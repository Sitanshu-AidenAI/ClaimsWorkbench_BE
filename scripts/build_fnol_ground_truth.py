...
from fnol_scenarios import SPECS  # noqa: E402
...
def main() -> None:
    ...
    flat = [{k: (v if not isinstance(v, list) else " | ".join(v)) for k, v in r.items()}
            for r in rows]
    ...
    md = ["# Matching ground truth", "",
          ...
          f"24 packs against the 12 policies in `../../policy/`.", "",
          "| Pack | Number on notice | Broker ref | Expected | Confidence |",
          "|---|---|---|---|---|"]
    for r in rows:
        num = f"`{r['mentioned_policy_number']}`" if r["mentioned_policy_number"] else "—"
        exp = (f"`{r['expected_matched_policy_number']}`"
               if r["expected_matched_policy_number"] != "NO_MATCH" else "**NO_MATCH**")
        md.append(f"| `{r['pack']}` | {num} | `{r['broker_reference']}` | {exp} "
    ...
    for r in rows:
        ...
        md += [f"### {r['pack']}", "",
               ...
               f"- **Policy number on the notice:** "
               + (f"`{r['mentioned_policy_number']}`" if r["mentioned_policy_number"] else "*not stated*"),
    ...
    print(f"wrote {len(rows)} rows to case_data/_ground_truth/MATCHING_GROUND_TRUTH.{{json,csv,md}}")
